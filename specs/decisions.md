...
Status: accepted (updated 2026-05-05: generic dispatcher pattern; nginx + Let's Encrypt; host `algo.telagri.com`). **"No rebuild" claim superseded by ADR 0022** — new CRAN deps require an image rebuild.
...
## 0021 — Strategic pivot: manual F-100 field-monitoring → algorithm-driven risk engine (ALE)
Status: accepted (2026-06-06)
Context: The original product — F-100 field monitoring + farmers list (`products_enabled` bit 1 "FieldMonitoring") — was built for banks around **manual, on-site** field monitoring: specialists visited fields and product managers hand-crafted charts and narratives per phase. This does not scale; operations are largely manual and bespoke per farmer/phase. TelAgri has hired an **agronomist** and a **GIS specialist** to move to a scalable, computational model.
Decision: The strategic product is now an **algorithm-driven agronomic-risk engine** (the ALE module). Algorithms transform agronomic risks/logic into numbers from three input classes: **crop phenology, weather data, and satellite imagery (Sentinel Hub — a future integration, not yet built)**. The **frost-risk** algorithm (already ported) is the **first of many**; more are in active development. The legacy F-100 / farmers / field-monitoring product is a **leftover** — it keeps running and its code is **not** removed, but it is not the strategic direction and gets no new investment without explicit reason.
Authoring workflow (the operating model): (1) **Agronomist** designs the risk logic + narrative — which data sources, what parameters to inject, how the result is calculated; (2) **GIS specialist** (R/Python + data-analytics expertise) implements it as an **R script** and verifies it with the agronomist; (3) on agreement it goes to the **engineer**, who **TS-ports it into ALE**, with the R parity service (ADR 0013) verifying the port matches R.
Consequences: New agronomic capability lands as ALE algorithms, not bespoke per-farmer monitoring. The bank-facing **delivery** of these results (orders, monitoring dashboards, orchard cash-flow) is a separate, **DEFERRED** effort — see [`modules/bank-delivery.md`](modules/bank-delivery.md) — to be designed only **after** the algorithm set is implemented and verified in ALE. Sentinel Hub satellite ingestion is a near-term build prerequisite for several algorithms.

## 0022 — Parity image rebuilds for new R deps; EC2 is a semi-permanent prod runtime (supersedes 0013's "no rebuild" claim)
Status: accepted (2026-06-07)
Context: ADR 0013 claimed "adding a new algorithm = drop a folder + `docker compose restart plumber`, no rebuild." This held only while algorithms shared the baked R packages (plumber/jsonlite/digest). heat-stress and insufficient-chill introduced **new CRAN deps** (`httr`, `dplyr`, `lubridate`, `ChillModels`), so the parity image **must be rebuilt** to add them — the "no rebuild" invariant is false in general.
Decision: The parity Dockerfile bakes a **generous base** of R deps (now httr/dplyr/lubridate/ChillModels on top of plumber/jsonlite/digest). Adding an algorithm whose deps are already baked still needs only `git pull && docker compose restart plumber`; adding one with a **new** CRAN package requires editing the Dockerfile dep line and an **image rebuild + redeploy** (SSM `docker compose build`). The EC2 is therefore a **semi-permanent production runtime** for the authoring/parity period, not a throwaway. gis-scripts is its **own repo** (`LashaShoniaTelagri/gis-scripts`), deployed to the host independently of the dashboard repo.
Consequences: New algorithms occasionally incur a heavy ARM image rebuild (dplyr/ChillModels compile; keep ~2GB swap). Bake speculative-but-likely deps up front to amortise. Everything else in 0013 (generic dispatcher, routes, HMAC, host, teardown-after-signoff) stands.
